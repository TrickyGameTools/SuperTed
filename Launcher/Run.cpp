...
namespace SuperTed {
	namespace Launcher {
		...
		static int Exe(std::string cmd) {
			...
			cmd = TReplace(cmd, '/', '\\'); // Guarantee cmd support in Windows!
#endif
			TQSG_Cls();
			TQSG_Color(255, 0, 0);
			j19gadget::GetDefaultFont()->Draw("External call in session...", floor(TQSG_ScreenWidth() / 2), floor(TQSG_ScreenHeight() / 2), 2, 2);
			TQSG_Flip();
		...
		static void ActNewProject(j19gadget*j,j19action a){ 
			...
			if (!FileExists(cmd)) {
				TQSE_Notify("The wizard (" + cmd + ") could not be found");
				return;
			}
			Exe(cmd);
